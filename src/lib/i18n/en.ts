export const en: Record<string, Record<string, string>> = {
  app: {
    title: 'Campus Resume Builder',
    subtitle: 'AI-Powered · Optimized for Campus Recruitment · One-Click Generate',
  },
  editor: {
    sections: 'Sections',
    language: 'Language',
    switchToEn: 'English',
    switchToZh: '中文',
    addSection: 'Add',
    deleteSection: 'Delete',
    moveUp: 'Move Up',
    moveDown: 'Move Down',
    save: 'Save',
    reset: 'Reset',
  },
  common: {
    addItem: 'Add item',
    delete: 'Delete',
    empty: 'No items yet',
  },
  layout: {
    title: 'Display',
    description: 'Hidden sections keep their data and only affect preview/export.',
    show: 'Show',
    hide: 'Hide',
    visible: 'Visible',
    hidden: 'Hidden',
    moveUp: 'Up',
    moveDown: 'Down',
    dragHandle: 'Drag to reorder',
  },
  sections: {
    personalInfo: 'Personal Info',
    education: 'Education',
    honors: 'Honors & Awards',
    experience: 'Internship',
    projects: 'Projects',
    campusActivities: 'Campus Activities',
    researchExperience: 'Research Experience',
    skills: 'Skills',
  },
  personalInfo: {
    nameZh: 'Name (Chinese)',
    nameEn: 'Name (English)',
    titleZh: 'Target Position (Chinese)',
    titleEn: 'Target Position (English)',
    email: 'Email',
    phone: 'Phone',
    locationZh: 'Location (Chinese)',
    locationEn: 'Location (English)',
    website: 'Website/GitHub',
    summaryZh: 'Summary (Chinese)',
    summaryEn: 'Summary (English)',
    gender: 'Gender',
    birthDate: 'Date of Birth',
    politicalStatus: 'Political Status',
    avatarUrl: 'Photo URL',
    avatarUpload: 'Choose local photo',
    avatarRemove: 'Remove photo',
    avatarPreview: 'Photo',
  },
  avatar: {
    invalidType: 'Only JPG, PNG, or WebP images are supported.',
    fileTooLarge: 'Image must be 5MB or smaller.',
    readFailed: 'Could not read the image. Choose it again.',
    cropTitle: 'Crop Photo',
    cropDescription: 'Crop to a centered 3:4 portrait before saving it to this resume.',
    cropConfirm: 'Use photo',
    cropCancel: 'Cancel',
    cropFailed: 'Could not crop the image. Choose it again.',
    cropping: 'Processing...',
    zoom: 'Zoom',
  },
  education: {
    schoolZh: 'School (Chinese)',
    schoolEn: 'School (English)',
    degreeZh: 'Degree (Chinese)',
    degreeEn: 'Degree (English)',
    majorZh: 'Major (Chinese)',
    majorEn: 'Major (English)',
    period: 'Period',
    gpa: 'GPA/Ranking',
    courses: 'Core Courses',
    coursesPlaceholder: 'Comma-separated, e.g. Data Structures, OS, Networks',
    descriptionZh: 'Description (Chinese)',
    descriptionEn: 'Description (English)',
  },
  honors: {
    titleZh: 'Award Name (Chinese)',
    titleEn: 'Award Name (English)',
    level: 'Level/Grade',
    levelPlaceholder: 'e.g. National, Provincial, University',
    period: 'Date',
    descriptionZh: 'Description (Chinese)',
    descriptionEn: 'Description (English)',
  },
  experience: {
    companyZh: 'Company (Chinese)',
    companyEn: 'Company (English)',
    roleZh: 'Position (Chinese)',
    roleEn: 'Position (English)',
    period: 'Period',
    descriptionZh: 'Description (Chinese)',
    descriptionEn: 'Description (English)',
    highlights: 'Achievements',
    addHighlight: 'Add Achievement',
  },
  projects: {
    nameZh: 'Project Name (Chinese)',
    nameEn: 'Project Name (English)',
    roleZh: 'Role (Chinese)',
    roleEn: 'Role (English)',
    period: 'Period',
    tech: 'Tech Stack',
    descriptionZh: 'Description (Chinese)',
    descriptionEn: 'Description (English)',
    link: 'Link',
    highlights: 'Project Highlights',
    addHighlight: 'Add highlight',
  },
  campusActivities: {
    organizationZh: 'Organization (Chinese)',
    organizationEn: 'Organization (English)',
    roleZh: 'Role (Chinese)',
    roleEn: 'Role (English)',
    period: 'Period',
    descriptionZh: 'Description (Chinese)',
    descriptionEn: 'Description (English)',
    highlights: 'Achievements',
    addHighlight: 'Add Achievement',
  },
  research: {
    institutionZh: 'Institution/Lab (Chinese)',
    institutionEn: 'Institution/Lab (English)',
    projectZh: 'Project/Topic (Chinese)',
    projectEn: 'Project/Topic (English)',
    roleZh: 'Role/Responsibility (Chinese)',
    roleEn: 'Role/Responsibility (English)',
    period: 'Period',
    descriptionZh: 'Research Overview (Chinese)',
    descriptionEn: 'Research Overview (English)',
    highlights: 'Achievements',
    addHighlight: 'Add Achievement',
    empty: 'No research experience yet. Click above to add.',
  },
  skills: {
    categoryZh: 'Category (Chinese)',
    categoryEn: 'Category (English)',
    items: 'Skills',
    addItem: 'Add Skill',
  },
  ai: {
    title: 'AI Assistant',
    generateBtn: 'Generate',
    generating: 'Generating...',
    jdPlaceholder: 'Paste the target job description (JD) here...',
    materialTitle: 'Personal Materials',
    parseBtn: 'Parse Material',
    parsing: 'Parsing...',
    applyBtn: 'Apply to Resume',
    discardBtn: 'Discard Changes',
    settings: 'AI Settings',
    provider: 'AI Provider',
    apiKey: 'API Key',
    model: 'Model',
    baseUrl: 'API URL',
    noMaterial: 'No materials added yet. Click "Add Material" to start.',
    addMaterial: 'Add Material',
    selectMaterial: 'Select Material',
    modeLabel: 'AI Mode',
    modeGenerate: 'Generate',
    modeOptimize: 'Optimize',
    modePolish: 'Polish',
    modeTranslate: 'Translate',
    targetRole: 'Target Role',
    targetRolePlaceholder: 'e.g. Frontend Developer',
    jdLabel: 'Job Description (JD)',
    materialsLabel: 'Personal Materials',
    materialsPlaceholder: 'Paste your personal experience, project descriptions, skills, etc...',
    resultSummary: 'AI Suggestion Summary',
    resultWarnings: 'Warnings',
    resultChanges: 'Modified Sections',
    privacyNotice: 'When using AI features, your resume content, job descriptions, and personal materials are sent to Google Gemini API via our server for processing. We do not store your data on the server.',
  },
  export: {
    title: 'Export',
    pdf: 'Export PDF',
    oneClickPdf: 'Export PDF',
    pdfGenerating: 'Generating PDF...',
    pdfSuccess: 'PDF exported',
    pdfFallback: 'Print fallback',
    pdfFailedUsePrint: 'PDF export failed. Try print fallback.',
    svg: 'Export SVG',
    json: 'Export JSON',
    importJson: 'Import JSON',
    popupBlocked: 'Please allow popups to export PDF',
    svgFailed: 'SVG export failed',
    importSuccess: 'Import successful',
    importFailed: 'Unable to parse JSON file',
    loadDemo: 'Load demo',
    loadDemoTitle: 'Load a fictional demo resume for preview. This overwrites current content.',
    loadDemoConfirm: 'Loading demo data will overwrite the current resume. Continue?',
    resetConfirm: 'Clear all resume content?',
  },
  workspace: {
    edit: 'Edit',
    aiAssist: 'AI Assist (Beta)',
    layout: 'Layout',
    export: 'Export',
  },
  templates: {
    classic: 'Classic',
    modern: 'Modern',
    minimal: 'Minimal',
    compact: 'Compact',
    switchTemplate: 'Switch Template',
  },
  emphasis: {
    expanded: 'Expanded',
    normal: 'Normal',
    compact: 'Compact',
    hidden: 'Hidden',
  },
  runtime: {
    shutdown: 'Stop server',
    shutdownTitle: 'Stopping…',
    shutdownConfirm: 'Stop the local dev server?',
    shutdownStarting: 'Server shutting down…',
    shutdownFailed: 'Shutdown failed. Stop the process in your terminal manually.',
    shutdownDone: 'Server stopped ✓',
    shutdownUnavailable: 'Unavailable in this environment.',
  },
};
